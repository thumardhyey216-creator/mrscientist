...
const SecondRevision = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Filter topics that have second_revision_date or have completed first revision
            const revisionTopics = data.filter(t => t.second_revision_date || t.first_revision === 'TRUE');
    ...
    const markRevisionComplete = async (topicId) => {
        try {
            const { error } = await supabase
                .from('topics')
                .update({ second_revision: 'TRUE' })
                .eq('id', topicId);

            if (error) throw error;

            setTopics(prev => prev.map(t =>
                t.id === topicId ? { ...t, second_revision: 'TRUE' } : t
            ));
        } catch (err) {
    ...
    const getTopicsForDate = (date) => {
        ...
        return topics.filter(t => {
            if (!t.second_revision_date) return false;
            return t.second_revision_date.split('T')[0] === dateStr;
        });
    };
    ...
    const weekCompleted = weekTopics.filter(t => t.second_revision === 'TRUE').length;
    const totalDue = topics.filter(t => t.first_revision === 'TRUE' && t.second_revision !== 'TRUE').length;
        ...
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold flex items-center gap-2">
                        <RotateCcw className="text-[var(--accent-cyan)]" />
                        2nd Revision Tracker
                    </h2>
                    <p className="text-[var(--text-secondary)]">Track your second revision of topics</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="glass-card p-4 text-center">
                    <Target className="mx-auto mb-2 text-[var(--accent-cyan)]" size={24} />
                        ...
                        return (
                            ...
                            >
                                <div className="text-center mb-2">
                                    ...
                                    <div className={`text-lg font-bold ${isToday ? 'text-[var(--accent-cyan)]' : ''}`}>
                                        {date.getDate()}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1">
                                    {dayTopics.slice(0, 3).map(topic => (
                                        <button
                                            ...
                                            className={`w-full text-xs p-1.5 rounded truncate text-left transition-all hover:scale-105 ${topic.second_revision === 'TRUE'
            ...
            <div className="glass-card p-6">
                <h3 className="text-lg font-semibold mb-4">Pending 2nd Revisions</h3>
                <div className="space-y-3">
                    {topics.filter(t => t.second_revision !== 'TRUE' && t.first_revision === 'TRUE').slice(0, 10).map(topic => (
                        <div
                            key={topic.id}
